<template>
  <div class="wrap">
    ...
    <div class="main">
      <ul class="form">
        <li>
          <span class="f_l">{{$t('phone.label.l3')}}</span>
          ...
          <input class="f_r" type="number" v-model="u_phone" :placeholder="$t('phone.phr.p3')">
        </li>
        <li>
          <span class="f_l">{{$t('phone.label.l2')}}</span>
          <input class="f_l code_input" type="text" v-model="u_code" :placeholder="$t('phone.phr.p2')">
          <span class="code_span f_r" :class="{f_c:btn_msg==$t('code')}" @click="getCode">{{btn_msg}}</span>
        </li>
        <li>
          <span class="f_l">{{$t('phone.label.l4')}}</span>
          <select class="f_l" v-model="p_type1">
            <option
              v-for="(item,index) in list"
              :key="index"
              :value="item.ph_pre_id+','+item.pre_code"
          ...
          <input class="f_r" type="number" v-model="u_phone1" :placeholder="$t('phone.phr.p4')">
        </li>
        <li>
          <span class="f_l">{{$t('phone.label.l2')}}</span>
          <input class="f_l code_input" type="text" v-model="u_code1" :placeholder="$t('phone.phr.p2')">
          <span
            class="code_span f_r"
            :class="{f_c:btn_msg1==$t('code')}"
            @click="getCode1"
          >{{btn_msg1}}</span>
        </li>
      </ul>
      <div class="tip">
        <router-link class="f_c" :to="{name:'changephone2'}">{{$t('phone.tip')}}？</router-link>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getCode1() {
      let that = this;
      let reg = /^(13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[89])\d{8}$/;
      if (reg.test(that.u_phone1)) {
        if (that.btn_msg1 == that.$t('code')) {
          that.btn_msg1 = "60"+that.$t('second');
          window.t1 = setInterval(function() {
            that.time1--;
            that.btn_msg1 = that.time1 + that.$t('second');
            if (that.time1 <= 0) {
              that.btn_msg1 = that.$t('code');
    ...
    sub() {
      let that = this;
      if (that.u_phone&&that.u_code&&that.u_phone1&&that.u_code1) {
        ...
        that
            .$http({
              ...
              data: {
                token: that.$store.state.user_info.token,
                old_phone: that.u_phone,
                new_phone: that.u_phone1,
                new_phone_code: that.u_code1,
                old_phone_code: that.u_code,
                pre_code_id:that.p_type1.split(",")[0],
                pre_code:that.p_type1.split(",")[1],
              }
            })
            .then(function(res) {
